...
import java.util.*;
import com.tsingshan.common.config.Global;
        ...
        if (rows > 0)
        {
            ...
            for (int i = 1; i < rows; i++) {
                ...
                for (int j = 0; j < cellNum; j++) {
                    ...
                    //Object object = convetType(fieldType, c);
                    field.set(entity,convetType(fieldType, c));
                    /*if (String.class == fieldType) {
                        ...
                        if (cell.getCellTypeEnum() == CellType.NUMERIC) {
                            ...
                            if (DateUtils.isValidDate(c,DateUtils.YYYY_MM_DD_HH_MM_SS)) {
                                if (DateUtils.isRightDateStr(String.valueOf(c),DateUtils.YYYY_MM_DD)) {
                                    Date date = DateUtils.dateTime("yyyy-MM-dd", String.valueOf(c));
...
//                   }
     ...
     * @return
     */
    private static Object convetType(Class clazz, String value) {
        if (String.class == clazz) {
            ...
            if (DateUtils.isRightDateStr(value,"yyyy-MM-dd")) {
                return DateUtils.dateTime("yyyy-MM-dd",value);
            } else if (DateUtils.isRightDateStr(value,"yyyy/MM/dd")) {
                return DateUtils.dateTime("yyyy-MM-dd",value);
            } else if (DateUtils.isRightDateStr(value,"yyyy-MM-dd HH:mm:ss")) {
                return DateUtils.dateTime("yyyy-MM-dd HH:mm:ss",value);
            }else if (DateUtils.isRightDateStr(value,"yyyy/MM/dd HH:mm:ss")) {
                return DateUtils.dateTime("yyyy-MM-dd HH:mm:ss",value);
            } else if (DateUtils.isRightDateStr(value,"yyyyMMdd")) {
                return DateUtils.dateTime("yyyyMMdd",value);
            }
        }
        return value;
   }
